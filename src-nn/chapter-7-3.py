...
В этой задаче мы будем применять L1-регуляризацию, а именно: припишем к "обычной" функции потерь
L(w) "хвост" +C(|w0|+|w1|), где в качестве константы С мы возьмем С=1.
После этого начаинаем обычную процедуру градиентного спуска.
...
Совет. Если возникли сложности с вычислением производной выражения, содержащей модуль, то посмотрите подсказку со следующего шага.

Lw = (w0 - 1)**2 + (w1 + w0 - 2)**2 + (2*w1 + w0 - 3)**2

'''
from sympy import diff, lambdify, symbols, pprint, Abs
...
def lostFn1(a0, a1, h, iterations):
    ...
    for i in range(iterations):
        pprint(d0 + getAbsDiff(a0))
        pprint(d1 + getAbsDiff(a1))

        l0 = lambdify([w0, w1], d0 +  getAbsDiff(a0))
        l1 = lambdify([w0, w1], d1 +  getAbsDiff(a1))
...
В этой задаче мы будем применять L1-регуляризацию, а именно: припишем к «обычной» функции потерь L(w) «хвост" +C(|w0|+|w1|), где в качестве константы С мы возьмем С=1.
После этого начинаем обычную процедуру градиентного спуска.
Для старта спуска возьмем a0=(0,0), h=0.1, причем мы предполагаем, что первая координата позиции «шарика» соответствует весу w0, а вторая координата соответствует весу w1.
Давайте сделаем два шага градиентного спуска и получим позицию a2.
В окошко ответа запишите вторую координату (которая соответствует весу w1) позиции a2.

Lw = (-w1 + w0 - 1)**2 + (w0)**2 + (w1 + w0 + 1)**2
'''

def lostFn2(a0, a1, h, iterations):
    w0, w1 = symbols('w0 w1')
    Lw = (-w1 + w0 - 1)**2 + (w0)**2 + (w1 + w0 + 1)**2
